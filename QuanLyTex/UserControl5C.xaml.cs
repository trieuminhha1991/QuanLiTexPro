...
namespace QuanLyTex
{
	...
	public partial class UserControl5C : System.Windows.Controls.UserControl
	{
		...
		public List<string> FilterId(string tex, bool? select)
		{
			try
			{
				List<string> list = new List<string>();
				...
				bool bt = false;
				bool vd = false;
				string exString = ExString.Text;
				string btString = BtString.Text;
				string vdString = VdString.Text;
				if (CauHoi.IsChecked == true)
				{
					ex = true;
				}
				if (BaiTap.IsChecked == true)
				{
					bt = true;
				}
				if (ViDu.IsChecked == true)
				{
					vd = true;
				}
				string str = @"\begin{";
