					...
					foreach (string path in listpath)
					{
						try
						{
							...
							foreach (string item in liststr)
							{
								...
								find.Text = "(" + item + ")([ ]{1,})([0-9]{1,3})";
								find.Execute(Wrap: WdFindWrap.wdFindStop, MatchWildcards: true, Replace: WdReplace.wdReplaceAll, ReplaceWith: @"\1 \3");
								range = doc.Content;
								find = range.Find;
						...
						try
						{
							...
							for (int i = list.Count - 2; i >= 0; i--)
							{
								try
								{
									...
									Range rangenew = doc.Range(listend[i], end);
									range.ParagraphFormat.Alignment = Microsoft.Office.Interop.Word.WdParagraphAlignment.wdAlignParagraphLeft;
									rangenew.Text = ".Lời giải\r\n";
								}
								catch { }
							}
							string pathitem = pathsave + @"\" + System.IO.Path.GetFileNameWithoutExtension(path) + ".docx";
							doc.Content.Font.Name = "Times New Roman (Headings)";
							doc.SaveAs(pathsave, Microsoft.Office.Interop.Word.WdSaveFormat.wdFormatDocumentDefault);
							if (AddPdf == true)
							{
								pathitem = pathsave + @"\" + System.IO.Path.GetFileNameWithoutExtension(path) + ".pdf";
								doc.SaveAs2(pathitem, WdSaveFormat.wdFormatPDF);
