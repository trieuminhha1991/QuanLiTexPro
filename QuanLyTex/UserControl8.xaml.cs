...
namespace QuanLyTex
{
	...
	public partial class UserControl8 : System.Windows.Controls.UserControl
	{
		...
		public async System.Threading.Tasks.Task FilterBasic_Acynce(bool? Hide, List<string> list,string name,string rx, string type, bool? color, bool? bold, bool? italic,string appstr,bool?sort,bool? sort1,bool?sort2,bool?sort3,bool?sort4,bool?sort5,bool? bankex,bool? Id6,bool?str2)
		{
			await System.Threading.Tasks.Task.Run(() =>
			{
				try
				{
					...
					classlist.mapId(document, Hide, list, app, rx, type, color, bold, italic);
					DateTime time = DateTime.Now;
					string TimeName = time.ToString("h.mm.ss");
					string path = appstr + @"\LuuFile" + @"\[" + name + "][" + TimeName + "].docx";
					if (sort == true || bankex == true)
					{
						classlist.mapSort(document, Hide, app, type, sort, sort1, sort2, sort3, Sort4.IsChecked, sort4, path, rx, color, bold, italic, bankex, Id6, str2);
					}
