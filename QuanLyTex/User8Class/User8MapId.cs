	...
	class User8MapId
	{
		...
		public void FilterId(Application app,string rx, string path, string Type, Document doc, bool? color, bool? bold, bool? italic,bool? Hide)
		{
			try
			{
				...
				find.Text = "("+Type + @")([ ]{1,4})([0-9]{1,3})";
				find.Font.Bold = 1;
				find.MatchWildcards = true;
				find.Execute(Format:true,Replace:WdReplace.wdReplaceAll,ReplaceWith:@"\1 \3");
				if(color==true)
				{
					range = doc1.Content;
					find = range.Find;
		...
		public void mapId(Document doc,bool?Hide,List<string> list,Application app,string rx, string type,bool? color,bool? bold, bool? italic)
		{
			try
			{
						...
						foreach (SortId item in listsort)
						{
							...
							foreach (datalist data in list)
							{
								try
								{
									...
									rangenew2.FormattedText=rangenew2.FormattedText;
